...
| `PENDING` | `ULG-25` | Added first-class pointer/input positioning controls with command routing and CLI validation. | Deliver M5-2 acceptance so cursor style and input flow can switch live and persist across restarts. | Extended command ID catalog for deterministic cursor/input actions, added app-shell getters/setters + route handlers + command registry entries, added CLI flags (`--cursor-style`, `--input-position`) with strict parsing, and expanded persistence/regression tests in `ulgen-app`. | `cargo test -p ulgen-command -p ulgen-app`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B). |
