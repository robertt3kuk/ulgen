...
| `PENDING` | `ULG-23` | Hardened permission-gating enforcement for interactive ACP terminals and cross-session protection paths. | Deliver M4-4 baseline controls so user-owned sessions cannot be disrupted by unauthorized agent actions. | Added ownership-before-read guards, refreshed interactive-state gating across control operations, and regression tests proving cross-session calls fail before backend output side effects. | `cargo test -p ulgen-acp`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B). |
