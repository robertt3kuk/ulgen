...
| `13006da` | `ULG-21` | Implemented ACP terminal bridge RPC surface and session-scoped terminal ownership lifecycle with bounded output and safety cleanup semantics. | Deliver M4-2 acceptance so ACP agents can create/read/wait/kill/release terminals through Ulgen transport contracts with auditable behavior. | Added terminal DTOs and JSON-RPC dispatch (`terminal/create|output|wait_for_exit|kill|release`), terminal error mapping (`-32005`), ownership enforcement across sessions, safe release and cancel cleanup behavior, UTF-8-safe output truncation, and transport/bridge tests for aliases/defaults, ownership rejection, and lifecycle flows. | `cargo test -p ulgen-acp`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B, both pass with no blocking findings). |
| `c151433` | `ULG-22` | Implemented interactive app mode primitives for ACP terminals with alternate-screen detection, raw input routing, and user/agent control arbitration. | Deliver M4-3 acceptance so full-screen apps remain usable under ACP without losing user control. | Added `terminal/input` RPC/method plumbing, interactive-mode detection from ANSI alternate-screen transitions, session-owned control arbitration (`agent` vs `user`) in bridge operations, and regression tests for interactive blocking/unblocking and lifecycle stability. | `cargo test -p ulgen-acp`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B). |
| `45e765f` | `ULG-23` | Hardened permission-gating enforcement for interactive ACP terminals and cross-session protection paths. | Deliver M4-4 baseline controls so user-owned sessions cannot be disrupted by unauthorized agent actions. | Added ownership-before-read guards, refreshed interactive-state gating across control operations, and regression tests proving cross-session calls fail before backend output side effects. | `cargo test -p ulgen-acp`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B). |

## M5 - Themes, Pointer, Polish, Beta

| Commit | Issue | What | Why | How | Validation |
|---|---|---|---|---|---|
| `4bd7029` | `ULG-24` | Added semantic theme system foundations with presets, mode resolution, and live/persisted app-shell theme application. | Deliver M5-1 acceptance so UI and terminal theme state can switch instantly and remain durable across restarts. | Extended settings schema with theme presets and semantic token bundles, added runtime theme resolver with system-mode fallback, exposed app-shell theme APIs for live resolution updates, and added persistence/regression tests in `ulgen-settings` and `ulgen-app`. | `cargo test -p ulgen-settings -p ulgen-app`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B). |
| `6eb9cfb` | `ULG-25` | Added first-class pointer/input positioning controls with command routing and CLI validation. | Deliver M5-2 acceptance so cursor style and input flow can switch live and persist across restarts. | Extended command ID catalog for deterministic cursor/input actions, added app-shell getters/setters + route handlers + command registry entries, added CLI flags (`--cursor-style`, `--input-position`) with strict parsing, and expanded persistence/regression tests in `ulgen-app`. | `cargo test -p ulgen-command -p ulgen-app`; `cargo test --workspace`; dual-loop xhigh subagent review process (Loop A + Loop B). |
