...
## 8) CodeRabbit + Codex Review Loop

Use CodeRabbit in prompt-focused mode before marking PRs ready:

1. Run prompt-only review on the current branch diff:
   - `./scripts/coderabbit_codex_review.sh --base master --type all`
2. For pre-commit local checks only:
   - `./scripts/coderabbit_codex_review.sh --type uncommitted`
3. If findings are returned, fix all actionable issues and rerun until clean.

Notes:
- This repo stores guidance in `AGENTS.md` (uppercase). The wrapper passes `-c AGENTS.md` so CodeRabbit always receives these instructions.
- If CodeRabbit returns rate-limit errors, wait for the cooldown window and rerun.
